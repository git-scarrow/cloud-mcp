...
// Oracle connection (using thin client - no initOracleClient needed)
// oracledb.initOracleClient(); // Not needed for thin client
...
exports.handler = async (event) => {
    ...
    try {
        // Demo mode check - if credentials are placeholders, simulate data
        const isDemoMode = process.env.ORACLE_USER === 'placeholder' || 
                          !process.env.ORACLE_USER || 
                          !process.env.ORACLE_PASSWORD ||
                          !process.env.ORACLE_CONNECT_STRING;
        
        if (isDemoMode) {
            console.log('Running in demo mode - simulating PhotoSight data...');
            return await runDemoMode(event, results, startTime);
        }
...
/**
 * Demo mode - simulate PhotoSight intelligence processing
 */
async function runDemoMode(event, results, startTime) {
    console.log('Simulating PhotoSight intelligence processing...');
    ...
    try {
        switch (processingType) {
            case 'quality_analysis':
                results.analysisInsights = await simulateQualityAnalysis();
                break;
            case 'equipment_performance':
                results.equipmentPerformance = await simulateEquipmentPerformance();
                break;
            case 'processing_optimization':
                results.processingOptimizations = await simulateProcessingOptimization();
                break;
            case 'full':
            default:
                results.analysisInsights = await simulateQualityAnalysis();
                results.equipmentPerformance = await simulateEquipmentPerformance();
                results.processingOptimizations = await simulateProcessingOptimization();
                results.qualityTrends = 1;
                break;
        }
        
        results.executionTime = Date.now() - startTime;
        console.log('Demo PhotoSight intelligence processing completed:', results);
        
        return {
            statusCode: 200,
            body: JSON.stringify({
                message: 'PhotoSight intelligence processing completed successfully (DEMO MODE)',
                results,
                note: 'This is simulated data. Configure Oracle credentials for real analysis.'
            })
        };
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({
                message: 'PhotoSight intelligence processing failed (DEMO MODE)',
                error: error.message,
                results
            })
        };
...
/**
 * Simulate quality analysis data
 */
async function simulateQualityAnalysis() {
    const demoQualityData = {
        type: 'quality_analysis',
        timestamp: new Date().toISOString(),
        data: {
            qualityByCamera: [
                { CAMERA_MAKE: 'Canon', CAMERA_MODEL: 'EOS R5', AVG_QUALITY: 8.5, AVG_SHARPNESS: 9.2, AVG_COMPOSITION: 7.8, PHOTO_COUNT: 245 },
                { CAMERA_MAKE: 'Sony', CAMERA_MODEL: 'A7R IV', AVG_QUALITY: 8.2, AVG_SHARPNESS: 8.9, AVG_COMPOSITION: 7.5, PHOTO_COUNT: 189 },
                { CAMERA_MAKE: 'Nikon', CAMERA_MODEL: 'Z9', AVG_QUALITY: 8.0, AVG_SHARPNESS: 8.7, AVG_COMPOSITION: 7.3, PHOTO_COUNT: 156 }
            ],
            qualityTrends: [
                { CAPTURE_DAY: new Date(Date.now() - 86400000).toISOString(), DAILY_AVG_QUALITY: 8.3, PHOTOS_ANALYZED: 67, AVG_PROCESSING_TIME: 234 },
                { CAPTURE_DAY: new Date(Date.now() - 2*86400000).toISOString(), DAILY_AVG_QUALITY: 8.1, PHOTOS_ANALYZED: 52, AVG_PROCESSING_TIME: 267 },
                { CAPTURE_DAY: new Date(Date.now() - 3*86400000).toISOString(), DAILY_AVG_QUALITY: 7.9, PHOTOS_ANALYZED: 48, AVG_PROCESSING_TIME: 289 }
            ],
            qualityIssues: [
                { QUALITY_ISSUE: 'Good Quality', ISSUE_COUNT: 387, PERCENTAGE: 78.2 },
                { QUALITY_ISSUE: 'Blurry', ISSUE_COUNT: 67, PERCENTAGE: 13.5 },
                { QUALITY_ISSUE: 'Overexposed', ISSUE_COUNT: 23, PERCENTAGE: 4.6 },
                { QUALITY_ISSUE: 'Underexposed', ISSUE_COUNT: 18, PERCENTAGE: 3.7 }
            ]
        },
        ttl: Math.floor(Date.now() / 1000) + (15 * 60)
    };
    ...
    await docClient.send(new PutCommand({
        ...
        Item: {
            cacheKey: 'quality_analysis',
            analysisData: JSON.stringify(demoQualityData),
            lastUpdated: new Date().toISOString(),
            ttl: demoQualityData.ttl
        }
    }));
    
    console.log('Demo quality analysis cached successfully');
    return 1;
}

/**
 * Simulate equipment performance data
 */
async function simulateEquipmentPerformance() {
    const demoEquipmentData = {
        type: 'equipment_performance',
        timestamp: new Date().toISOString(),
        data: {
            lensPerformance: [
                { LENS_MODEL: 'RF 24-70mm f/2.8L IS USM', FOCAL_LENGTH: 50, APERTURE: 2.8, AVG_SHARPNESS: 9.1, AVG_QUALITY: 8.7, USAGE_COUNT: 89 },
                { LENS_MODEL: 'FE 70-200mm f/2.8 GM OSS', FOCAL_LENGTH: 135, APERTURE: 2.8, AVG_SHARPNESS: 8.9, AVG_QUALITY: 8.5, USAGE_COUNT: 67 },
                { LENS_MODEL: 'NIKKOR Z 85mm f/1.8 S', FOCAL_LENGTH: 85, APERTURE: 1.8, AVG_SHARPNESS: 9.3, AVG_QUALITY: 8.9, USAGE_COUNT: 45 }
            ],
            isoPerformance: [
                { ISO_RANGE: '100-400', AVG_QUALITY: 8.9, AVG_NOISE: 0.12, PHOTO_COUNT: 234 },
                { ISO_RANGE: '401-800', AVG_QUALITY: 8.5, AVG_NOISE: 0.18, PHOTO_COUNT: 189 },
                { ISO_RANGE: '801-1600', AVG_QUALITY: 7.8, AVG_NOISE: 0.31, PHOTO_COUNT: 98 },
                { ISO_RANGE: '1601-3200', AVG_QUALITY: 7.2, AVG_NOISE: 0.52, PHOTO_COUNT: 34 }
            ]
        },
        ttl: Math.floor(Date.now() / 1000) + (15 * 60)
    };
    ...
    await docClient.send(new PutCommand({
        ...
        Item: {
            cacheKey: 'equipment_performance',
            analysisData: JSON.stringify(demoEquipmentData),
            lastUpdated: new Date().toISOString(),
            ttl: demoEquipmentData.ttl
        }
    }));
    
    console.log('Demo equipment performance cached successfully');
    return 1;
}

/**
 * Simulate processing optimization data
 */
async function simulateProcessingOptimization() {
    const demoOptimizationData = {
        type: 'processing_optimization',
        timestamp: new Date().toISOString(),
        data: {
            processingTimes: [
                { ANALYSIS_TYPE: 'Quality Assessment', AVG_PROCESSING_TIME: 234, MIN_PROCESSING_TIME: 156, MAX_PROCESSING_TIME: 467, ANALYSIS_COUNT: 495 },
                { ANALYSIS_TYPE: 'Face Detection', AVG_PROCESSING_TIME: 89, MIN_PROCESSING_TIME: 45, MAX_PROCESSING_TIME: 234, ANALYSIS_COUNT: 312 },
                { ANALYSIS_TYPE: 'Object Recognition', AVG_PROCESSING_TIME: 345, MIN_PROCESSING_TIME: 234, MAX_PROCESSING_TIME: 789, ANALYSIS_COUNT: 267 }
            ],
            processingStatus: [
                { PROCESSING_STATUS: 'COMPLETED', STATUS_COUNT: 467, PERCENTAGE: 89.2 },
                { PROCESSING_STATUS: 'IN_PROGRESS', STATUS_COUNT: 34, PERCENTAGE: 6.5 },
                { PROCESSING_STATUS: 'FAILED', STATUS_COUNT: 23, PERCENTAGE: 4.3 }
            ]
        },
        recommendations: [
            {
                type: 'performance',
                priority: 'medium',
                issue: 'Object Recognition processing averaging 345ms',
                recommendation: 'Consider GPU acceleration or algorithm optimization',
                expectedImprovement: '40-50% processing time reduction'
            },
            {
                type: 'reliability',
                priority: 'low',
                issue: 'Processing failure rate at 4.3%',
    ...
    await docClient.send(new PutCommand({
        ...
        Item: {
            cacheKey: 'processing_optimization',
            analysisData: JSON.stringify(demoOptimizationData),
            lastUpdated: new Date().toISOString(),
            ttl: demoOptimizationData.ttl
        }
    }));
    
    console.log('Demo processing optimization cached successfully');
    return 1;
}